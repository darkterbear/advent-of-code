...
def output(machine, value):
    ...
    if len(outputBuffers[machine]) == 3:
        ...
        if packet[0] == 255:
            # print('update nat packet', packet)
            nat_packet = packet
        else:
...
def getInput(machine):
    ...
    if all([count > 1 for count in idle]) and all([not queue for queue in inputs]):
        print('nat send:', nat_packet[1:])
        inputs[0].extend(nat_packet[1:])
    return -1
