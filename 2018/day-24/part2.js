...
let groupsCopy = JSON.stringify(groups)
...
let calcOutcome = () => {
	...
	return {
		alive: groups.reduce((a, c) => a + c.numUnits, 0),
		team: groups[0].team
	}
...
let boost = 70
while (true) {
	console.log('test boost ', boost)
	groups = JSON.parse(groupsCopy)
	for (let i = 0; i < groups.length; i++) {
		if (groups[i].team === 'immune') {
			groups[i].attack.dmg += boost
		}
	}
